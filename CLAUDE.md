...
- **数据跨越信任边界**（外部 API 响应、DB 存储的外部数据）进入新的执行上下文（HTML / Shell / SQL）时，必须经过该上下文对应的转义或参数化处理。
- **HTML 渲染**：所有写入 `innerHTML` 的变量必须先经过 HTML 转义，无论数据来源是否「内部」。
- **触发 `/code-review` 的时机**：新增数据从后端流向 HTML 渲染、或新增对外暴露的 API 端点时，以及每次部署前。
